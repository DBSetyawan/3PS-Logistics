                            ...
                            <tbody>
                                @foreach($data_customer_transport as $item_rows)
                                  <tr class="odd gradeX">
                                      ...
                                      <td style="width: 17%;">{{ $item_rows->customers['name'] }}</td>
                                      ...
                                      <td style="width: 15%;">Rp. {{ number_format($item_rows->price, 0) }}</td>
                                      <td style="width: 10%;"><input type="checkbox" id="check_alert_item_customer" name="check_alert_item_customer[]" value="{{$item_rows->id}}"></td>
                                  </tr>
                                @endforeach()
                        </tbody>
                    </table>
...
<script src="{{ asset('js/system_alert_itemlist.js') }}"></script>
   ...
   $("#check_alert_item_customer").on("click", function() {
    let state = $(this).is(':checked');
    const id_item = $('#check_alert_item_customer').val();
   if (!state==false) {
        let flag=1; 
        Swal({
        ...
        cancelButtonText: 'Batal'
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: "get",
                    url: "{{ url('/alert-item-customer-tc-update/find') }}/"+id_item,
                    dataType: "json",
                    data: {
                        id:id_item,
                        flag:flag,
