       ...
       <div class="row-fluid">
                    ...
                    <table class="table table-striped table-bordered" id="sample_1">
                        <thead>
                            <tr>
                                ...
                                <th>Capacity</th>
                                <th>Tonase</th>
                                {{--s <th>Made by users</th> --}}
                                <th>Created At</th>
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                          @foreach($dp as $item_rows)
                            <tr class="odd gradeX">
                                <td style="width: 9%;">{{ $item_rows->id }}</td>
                                <td style="width: 36%;">{{ $item_rows->name }}</td>
                                <td style="width: 20%;">{{ $item_rows->capacity }}</td>
                                <td style="width: 20%;">{{ $item_rows->tonase }}</td>
                                {{-- <td style="width: 80%;">{{ $item_rows->users['name'] }}</td> --}}
                                <td style="width: 15%;">{{ $item_rows->created_at }}</td>
                                {{-- <td style="width: 60px;">
                                ...
                                @endphp
                                <td style="width: 10px;">
                                        <a href="{{ route('show.data.modas', array($some, $encrypts))}}"><button class="btn btn-primary"><i class="icon-pencil"></button></i></a>
                                  </td>
                            </tr>
                          @endforeach()
                        </tbody>
        ...
        <div class="modal fade" id="add_item" data-keyboard="false" data-backdrop="static" tabindex="-1" role="dialog"
                ...
                <div class="modal-body">
                    <form class="form-horizontal" id="add_item" method="post" action="{{url('modasv')}}">
                        ...
                        <label class="control-label" style="text-align: end">Sub Service</label>
                            <div class="controls">
                                <select class="form-control validate[required]" id="sub_service" name="sub_service" required>
                                </select>
                            </div>
                        </div>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input class="input-large" type="text" maxlength="30" id="itemid" name="itemid">
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label" style="text-align: end">Capacity</label>
                            <div class="controls">
                                <input class="input-large" type="text" maxlength="30" id="capc" name="capc">
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label" style="text-align: end">Tonase</label>
                            <div class="controls">
                                <input class="input-large" type="text" maxlength="30" id="ton" name="ton">
                            </div>
                        </div>
                </div>
                <div class="modal-footer">
                    ...
                    {{-- <input type="button" id="btnsave" class="btn btn-primary">Save</input> --}}
                </div>
            </div>
        </form>
...
<script src="{{ asset('js/offline/indexdb/idb.js') }}"></script>
<script src="{{ asset('js/offline/modas/modas.js') }}"></script>
<script src="{{ asset('js/offline/service-workers.js') }}"></script>
...
integrity="sha256-CGeJcrPLTi6rwHUfGazTyGhlRmLVJ4RIX79X1FmNQ3k=" crossorigin="anonymous"></script>
    ...
    //     pos_trx_id = document.querySelector("#sub_service").value;
    //     var pos_trx = {
    //         'sub_services':sub_service,
    //         'item_id':document.querySelector("#item_id").value,
    //         'capc':document.querySelector("#capc").value,
    //         'ton':document.querySelector("#ton").value
    //     };
    ...
    //         .then(function(sw)
    //         {
    //             idb_pos_trx.set(pos_trx)
    //             .then(function(){
    //                 sw.sync.register('sync-new-modas');
    //             });
    //         });
    //     }
    //     else
