@extends('admin.layouts.master', array('modaid' => session()->get('modaid')))
@section('head')
                ...
                <div class="widget red">
                    ...
                    <div class="widget-body">
                      ...
                      <div class="modal-footer">
                          FORM DETAIL MODA: <strong>{{ $lm->name }}</strong>
                        </div>
                        <!-- BEGIN FORM-->
                <form class="form-horizontal" id="add_item" method="POST" action="{{route('modasv.update', $lm->id)}}">
                        ...
                        <label class="control-label" style="text-align: end">Sub Service</label>
                            <div class="controls">
                                <select class="form-control validate[required]" id="sub_service" name="sub_service" required>
                                    @foreach($allmoda as $a)
                                        <option value="{{ $a->id }}"  @if($a->id==$lm->sub_service_id_fk) selected='selected' @endif >{{ $a->name }}</option>
                                    @endforeach()
                                </select>
                            </div>
                        </div>
                        ...
                        <label class="control-label" style="text-align: end">Tonase</label>
                            <div class="controls">
                                <input class="input-large" type="text" maxlength="30" name="ton" value="{{ $lm->tonase }}" />
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label" style="text-align: end">Capacity</label>
                                <div class="controls">
                                <input class="input-large" type="text" maxlength="30" name="capc" value="{{ $lm->capacity }}" />
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-primary">Update</button>
                            <a class="btn btn-success" href="{{ route('master.data.modas', session()->get('id')) }}">Back To List</a>
                        </div>
                    </div>
                </form>
            </div>
