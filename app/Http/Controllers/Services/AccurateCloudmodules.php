...
class AccurateCloudmodules implements AccuratecloudInterface
{
    ...
    */
    ...
    const signatureSecretKey = "02aa94e147a5be0bec92caf4ec97ac3f";
    const DBINDEXACCURATE__ = 146584; //3PE

    /**
     * @method APIs webhooks signature hooks 3PS broadcasting
     * 
     */
    const PROTOCOL_WEBHOOK = "http://devyour-api.co.id/webhooks";
     ...
     * 
     */
    const API_ACCURATE_BRANCH_LIST = "https://public.accurate.id/accurate/api/branch/list.do";
    ...
    */
    ...
    public function __construct(Request $request, Batchs_transaction_item_customer $batch_item)
    {
        ...
        $this->dates = Carbon::Now()->format('d/m/Y');
        ...
        try
            {
                ...
                //     );
                    ...
                    $client = new Client(['handler' => HandlerStack::create($handler), 'base_uri' => self::API_ACCURATE_AUTHORIZE_URL]);
                ...
                $response = $client->post(
                        ...
                        [
                            ...
                            [
                                ...
                                // 'id' => session('sessionID_accurate')[1] //3PE
                                'id' => isset(Auth::User()->oauth_accurate_company) ? Auth::User()->oauth_accurate_company : 146583
                                // '_ts' => $array_fetch_asort["_ts"],
                                // 'sign' => $signature
                            ]
                        ]
            ...
            $response = $client->post(
                    ...
                    [
                        ...
                        'form_params' => 
                                [
                                    // 'session' => $array_fetch_asort['session'],
                                    'branchId' => $array_fetch_asort['branchid'],
                                    'customerNo' => $array_fetch_asort['customerNo'],
                                    'name' => $array_fetch_asort['name'],
                                    'transDate' => $array_fetch_asort['transDate'],
...
// dd($datax[0]['detailNotes']);
        ...
        $fetchBatchItemOrderDetail = $this->batch_item
                ...
                //                         array(
                //                         'method'  => 'POST',
                //                         'header'  => "Authorization: bearer " . Auth::User()->oauth_accurate_token . "\r\n" ."Content-Type: application/x-www-form-urlencoded\r\n",
    ...
    public function FuncOpenmoduleAccurateCloudResetDetailItem($codeshipment, $branch_id, $idItem, $itemNo, $listerFetchIDPRIMARY, $itemID, $transport_id){

        try {

            foreach($listerFetchIDPRIMARY->getData("+")["d"]["detailItem"] as $keys => $Number){
                   
                $accurate[] = $Number["item"]["no"];

            }

                $rd = Batchs_transaction_item_customer::with(['transportsIDX','masterItemIDACCURATE.masteritemtc'])
                    ->whereIn("transport_id", [$transport_id])->whereIn('itemID', $itemID)->get();

                foreach($rd as $k => $v){

                    $internal[] = $v['masterItemIDACCURATE']['itemID_accurate'];

                }

                // dd($accurate, $internal);die;

                $mixed = ! array_intersect((array)$accurate, (array)str_replace('"',"", end($itemNo))) ? false : true;

                $status = $mixed === true ? "found" : "not found";
    ...
    public function FuncOpenmoduleAccurateCloudUpdateSalesOrders($itemIDaccurate, $branch_id, $dataPemesan, $idTransports, $detailNotes, $number, $id, $detailIDItem, $kuantitas, $price, $itemIDinternalaccurate, $idAccurateSH){

        try {

                $datax = collect([
                            [
                                'id_transport' => $idTransports, //in process [R::array]
                                'itemIDaccurate' => $itemIDaccurate, //in process [R::array]
                                'qty' => $kuantitas, //in process [R::array]
                                'detailNotes'=> $detailNotes, //in process [R::array]
                                'customerNo' => $dataPemesan,
                                'Price' => $price, // in process [R::array]
                                'idInternalAccurateSH' => $idAccurateSH, // id internal SO accurate
                                'idItem' => $itemIDinternalaccurate, //in process [R::array]
                                'branch_id' => $branch_id, //in process [R::branchId]
                            ]
                        ]
                    )
                ;
                ...
                $fetchBatchItemOrderDetail = $this->batch_item
                ->with(['transportsIDX','masterItemIDACCURATE.masteritemtc'])
                    ->whereIn('transport_id', [$idTransports])
                        ->get();

                foreach ($datax as $key => $value) {
                    $ls[] = [
                        'transport_id' => $value['id_transport'],
                        'itemID' => end($value['idItem']),
                        'itemIDacc' => end($value['itemIDaccurate']),
                        'qty' => end($value['qty']),
                        'detailnotes' => end($value['detailNotes']),
                        'harga' => end($value['Price']),
                        // 'cash_discount' => ($values[$key]['detailNotes'] === null) ? 0 : $values[$key]['detailNotes']
                    ];
                }
                        ...
                        foreach ($datax as $key => $values) {
                            foreach ($ls as $k => $v) {
                                $mArray[] = [
                                    'data['.(Int)$key.'].id' => (Int) $values['idInternalAccurateSH'],
                                    'data['.(Int)$key.'].customerNo' => $values['customerNo'],
                                    'data['.(Int)$key.'].branchId' => $values['branch_id'],
                                    'data['.(Int)$key.'].description' => implode("\n", $values['detailNotes']),
                                    'data['.(Int)$key.'].transDate' => $this->dates,
                                    'data[0].detailItem['.(Int)$k.'].itemNo' => str_replace('"','',$v['itemIDacc']),
                                    'data[0].detailItem['.(Int)$k.'].unitPrice' => $v['harga'],
                                    'data[0].detailItem['.(Int)$k.'].quantity' => $v['qty'],
                                    'data[0].detailItem['.(Int)$k.'].detailNotes' => $v['detailnotes'],
                                ];
                                ...
                                $arrys[] = [
                                    ...
                                    'data['.(Int)$key.'].description',
                                    'data['.(Int)$key.'].transDate',
                                    'data[0].detailItem['.(Int)$k.'].itemNo',
                                    'data[0].detailItem['.(Int)$k.'].unitPrice',
                                    'data[0].detailItem['.(Int)$k.'].quantity',
                                    'data[0].detailItem['.(Int)$k.'].detailNotes'
                                ];
                            }
                        }

                        $needles = Arr::flatten($arrys);
                        array_walk_recursive($mArray, function ($v, $k) use (&$sArray,$needles) {
                            if (in_array($k, $needles)) {
                                $sArray[$k]=trim($v);
                            }
    ...
    public function FuncOpenmoduleAccurateCloudSaveSalesOrders($number, $branch_id, $customerno, $itemNo, $transDate, $harga, $quantity, $transport_id, $detailnotes){
                    ...
                    $values[$key] = $value;
                        foreach($fetchBatchItemOrderDetail as $keys => $thisdataBatchItemTransports){
                            ...
                            $mArray[] = [
                                    ...
                                    'data['.(Int)$key.'].branchId' => $branch_id,
                                    'data['.(Int)$key.'].customerNo' => $values[$key]['customerNo'],
                                    'data['.(Int)$key.'].transDate' => $values[$key]['transDate'],
                                    'data['.(Int)$key.'].description' => implode("\n", $values[$key]['detailNotes']),
        ...
        try 
            {
                ...
                //         yield $client->postAsync('http://devyour-api.co.id/webhook');		
                //     })();
    ...
    public function FuncOpenmoduleAccurateCloudUpdateBarangJasaVendors($id, $minimumQuantity, $Cost, $name){
