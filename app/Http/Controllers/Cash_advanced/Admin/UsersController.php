...
class UsersController extends Controller
{
    ...
    public function searchbranchautomatic(Request $request, $id)
    {
        ...
        $companysbranch = Company_branchs::orderby('branch', 'asc')->whereIn('company_id', [$id])->where('branch', 'like', '%' .$search. '%')->limit(7)->get();
...
// $teamRole = $team->pivot->teamRole;
    ...
    public function add_companyenv__(Request $request, Companies $comp)
    {
    ...
    public function add_branchenv__(Request $request, Company_branchs $brnch)
    {
        ...
        if (Roles::where('name', $cek_roles)->count() > 0) {
            ...
            $update = User::findOrFail(Auth::User()->id);
            $update->company_branch_id = $branch->id;
            $update->save();
    ...
    public function LoadBranchwithcompanysuperuser($id)
    {
        ...
        $companysbranch = Role_branch::with('modelhasbranch.company')->whereHas('modelhasbranch.company', function (Builder $query) use ($id,$names) {
    ...
    public function updateSettingUserBranch(User $pengguna, $company, $branch, Role_branch $role_branch)
    {
        ...
        session(['id' =>$fetch_users->branch_id]);
        
        return response()->json($fetch_users->branch_id);
    }
    ...
    public function createUserNew($branch_id, Request $request)
    {
        ...
        if (Gate::allows('developer')) {
            ...
            return response()->json($data);
                    ...
                    if ($data->isEmpty()) {
                        ...
                        foreach (Auth::User()->roles as $name_role) {
                            $prefixRoles = $name_role->id;
                        }

                        $ambil_branch_id_users = $rbranch->with('modelhasbranch.company')->whereIn('role_id', [$prefixRoles])->get();
